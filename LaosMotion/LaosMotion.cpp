...
signed long bitmap[BITMAP_SIZE];
signed long bitmap_width=0; // nr of pixels
signed long bitmap_size=0; // nr of bytes
signed char bitmap_bpp=1, bitmap_enable=0;
    ...
    while (true) {
        if (mot->qstate == NEW) {
            if (mot->filename) {
                ...
                FILE *in = sd.openfile(mot->filename, "r");
                while (!feof(in))
                {
                    while (!mot->ready()) osDelay(500);
            ...
            if (mot->qstate == DONE && (! mot->filename)) mot->qstate = IDLE;
            osDelay(1000);
        }
    }
}
