...
typedef enum { NEW, BUSY, DONE, IDLE } QueueState;
...
public:
  ...
  char *filename; // file being cut
  QueueState qstate; // status of the queue (planner-filler)
private:
  
};
void queue_thread(void const *args); // queue thread to move file into planner
